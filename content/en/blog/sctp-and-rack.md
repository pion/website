---
title: Pion adds RACK to SCTP
Description: An improved loss protocol for SCTP
date: 2025-11-xx
...
SCTP stands for Stream Control Transmission Protocol. At a basic level, SCTP is designed to be reliable, handle de-duplication of packets, and handle packets which may ordered or unordered. Beyond transporting messages, SCTP can also setup a connection between users. On a deeper level, SCTP includes native support for multiplexing: multiple applications can take advantage of a single transport connection. SCTP also supports multi-homing, which is an automatic fail-over switch from the primary connection to a secondary connection.

At the most basic level, it lets you reliably send information from computer to another without any complications.
...
Imagine a scenario where two people are texting when one person remembers a picture that they want to send. As they text back and forth, an image gets uploaded, which takes some time to get sent. SCTP can handle multiple things going on at the same time, and doesn't delay any messages from sending just because an image is being uploaded! Thanks to SCTP, text messages can be safely delivered to each person and nothing in their conversation was lost in transit or delayed just because something else is being transferred at the same time as their messages.

Building on this idea, users can share larger files between each other. This includes anything: birthday videos, audio recordings, even boring paperwork; anything that's a file can be sent!
...
SCTP is also compatible with web browsers and has found use in AI applications and cryptocurrency related technologies. Additionally, payment verification can similarly benefit from secure and fast communication.
...
In WebRTC, the ICE protocol connects users and the DTLS protocol establishes a secure connection, at which point SCTP is then used to securely transfer data. In an ideal setup, data that's sent should *just work*. Unfortunately that isn't how things tend to pan out, as issues eventually crop up. Packets get dropped, the network jitters, the computer stutters, or the coffee machine didn't start when you thought you started it. That's why it's important to have a backup plan for when things go wrong.

## How SCTP Deals With Loss

SCTP was designed with this in mind and has a two built in recovery strategies for when networking goes wrong.

The first is called "fast retransmission". The receiver detects if a chunk of data is missing in the transmission. If so, then the receiver notifies the sender that the specific id of the missing chunk was missing. If the sender receives 3 reports of a missing chunk where all 3 reports are referring to the same chunk id, then the sender will assume that the chunk has been lost and it resends the chunk.
...
Both of these loss recovery strategies are used by SCTP to try to ensure that any lost data is detected and retransmitted as quickly as possible. At the time of writing, [Pion's implementation of SCTP](https://github.com/pion/sctp/tree/b1a66a4) used these two mechanisms for loss recovery.
...
In February 2021, [RFC 8985: The RACK-TLP Loss Detection Algorithm for TCP](https://www.rfc-editor.org/rfc/rfc8985.html) was published. This was a completely new loss detection algorithm that focused on actively keeping track of network statistics and more active timer-based statistics in order to remain adaptive to ever-changing network conditions. RACK's improvements over SACK and fast retransmission in TCP were enticing enough for the Linux, Windows, and FreeBSD to all implement it for TCP.
...
The authors of RACK in [RFC 8985](https://www.rfc-editor.org/rfc/rfc8985.html) has examples of situations where RACK would improve SCTP and TCP during loss recovery.
...
In the above scenario, Tail Loss Probing enables the sender to quickly know if 🍎🍌🥕🥔 were successfully received. Since the sender only received an acknowledgement (ack) for 🍎 from the receiver, the TLP's timer triggers since it didn't receive an ack for 🍌, 🥕, and 🥔. The TLP then can then resend the last packet in the segment as an efficient way to:

1) Retransmit data that the receiver would have to receive down the line anyways. The alternative would be to send an empty packet, receive an ack, *then* send a missing packet. This handles both at once, and can potentially save an RTT if *only* the last packet is missing in a segment.
2) Allow the receiver to ack any earlier missing packets in the sequence if there were other issues due to networking, temporary stutter or freeze, etc.
3) Check receiver responsiveness or if there's a network issue. Note that this is different from (2) as the receiver could potentially never respond with an ack.

The receiver then confirms that it has 🥔 by sending a selective ack (SACK) to the sender which tells the sender that it's received one of the packets from the segment, but not all of them. At this point, the sender has an ack for 🍎 and a SACK 🥔, which means that it can determine that 🍌 and 🥕 must be missing. The sender then notes that 🍌 and 🥕 have been lost once. It then retransmits 🍌 and 🥕. In the example, 🍌 happens to get dropped by the network whereas 🥕 is sent and received successfully. The receiver then replies with a SACK for 🥕 and 🥔, at which point the sender then can determine that 🍌 was lost a second time. Finally, the sender retransmits 🍌, which fortunately doesn't get dropped, and the receiver sends an ack for 🥔 (note the ack is for the last packet in the segment which implies that all previous packets have been received).

Side note: keeping track of the number of times that a packet has been lost is important as [RFC 8985](https://www.rfc-editor.org/rfc/rfc8985.html) refers to the cubic congestion control algorithm which is described in [RFC 9438](https://datatracker.ietf.org/doc/rfc9438/) (note that RFC 8985 refers to the previous version of RFC 9438, which is RFC 5681).

### SCTP RTO vs RACK RTO

In this example, the authors of [RFC 8985](https://www.rfc-editor.org/rfc/rfc8985.html) show how without RACK, SCTP and TCP would reduce spuriously retransmissions when there are retransmission timeouts (RTO). In this case, each food icon represents an entire segment instead of a packet.
...
Without RACK: the receiver then requests a retransmit from the sender, at which point the sender believes that A, B, and C are all missing, causing it to retransmit A, B, and C.
...
Here, RACK makes it so only 🥐 is marked as lost since the RTO expires. 🍳 and 🥭 aren't marked as lost because the ack is received before the respective RTO's expire. Therefore, only 🥐 is retransmitted, as the timer for 🍳 and 🥭 would re-arm if it receives an ack for the retransmitted 🥐.
...
1. Detect any packet losses as quickly as possible by utilizing time-based acknowledgements of segmented data and inferences from network statistics.
...
Keep an eye out for a new post with results of our SCTP implementation with real world data soon!
